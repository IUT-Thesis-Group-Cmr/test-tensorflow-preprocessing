...
if __name__ == '__main__':
    # path = 'methodology/data/dailydialog'
    # ext = '.json'
    # print(os.listdir(path))
    # train, valid, test = create_utterances(path + '/train' + ext, 'train'), \
    #                      create_utterances(path + '/valid' + ext, 'valid'), \
    #                      create_utterances(path + '/test' + ext, 'test')
    ...
    """
    # tv.tfidf_x_train = tv.tfidf_x_train.reshape(tv.tfidf_x_train.shape[0], tv.tfidf_x_train.shape[1], 1)
    # print(tv.tfidf_x_train.shape)
    # x =
    # x = x.reshape(x.shape[0], x.shape[1], 1)

    # tts_train = TrainTestSplit(tv.tfidf_x_train.todense(), df['emotion_index'],
    #                            test_set_ratio=0.15)
